...
def test_segment_conveyor_belt():
    ...
    if result["segmentation"]:
        assert isinstance(result["segmentation"][0], dict), "Segmentation items should be dictionaries"
        assert "mask" in result["segmentation"][0], "Segmentation items should contain 'mask' key"
    
    assert isinstance(result["output_image_path"], str), "Output image path should be a string"
    assert result["output_image_path"].endswith(".jpg"), "Output image should be a JPEG file"
...
def test_segment_and_visualize():
    ...
    assert isinstance(result, dict), "Result should be a dictionary"
    assert 'segmentation_results' in result, "Result should contain 'segmentation_results'"
    assert 'visualized_image_path' in result, "Result should contain 'visualized_image_path'"
