...
from .image_processing import load_and_process_image, save_processed_image, calculate_length
...
def detect_conveyor_sections(image_path: str) -> ConveyorSectionResult:
    ...
    detections = owl_v2_image("straight conveyor belt, curved conveyor belt", image, box_threshold=0.1)
    straight_sections = [d for d in detections if d['label'] == 'straight conveyor belt']
    curved_sections = [d for d in detections if d['label'] == 'curved conveyor belt']

    total_straight_length = sum(calculate_length(section, width, height) for section in straight_sections)
    total_curved_length = sum(calculate_length(section, width, height) for section in curved_sections)

    image_with_boxes = overlay_bounding_boxes(image, detections)
    visualization_path = save_processed_image(image_with_boxes, "conveyor_belt_detection.jpg")
